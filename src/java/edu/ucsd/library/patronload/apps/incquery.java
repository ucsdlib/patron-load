 ...
 * incquery.java
 *
 * This is the Java port of "darwin.inc.query", the perl version
 *
 * Created on June 20, 2002, 12:32 PM
 */
...
public class incquery {

    /** Creates new incquery */
    public incquery() {
    }

    /**
     * Method to get yesterday's Date. This is used to retrive the records that
     * have been added/changed since yesterday
     */
    ...
    public static void main(String[] args) {
        ...
        if ((args == null) || (args.length < 2)) {
            System.out
                    .println("\nSyntax: java incquery [pathToProperties] [fileToWrite]");
        } else {
    ...
    public static void getRawData(String pathToProperties, PrintWriter pw) {
        ...
        */
        ...
        try {
    		...
    		"ON S.stu_pid = I.stu_pid  LEFT JOIN affiliates_dw.affiliates_safe_attributes SA ON S.stu_pid = SA.pid " +
            ...
            String query = "select "
                    ...
                    + "(S.stu_pid *= E.stu_pid) and " + "(S.stu_pid *= I.pid) and "
            ...
            */        	
            try {
                ...
                FileUtils.stringToFile(query, marcFilesDir
                        + "inc_query_undergrad_student.sql");
            } catch (IOException ioe) {
            }
...
//            	 iterations = 3; will be uncomment on august 16
            ...
            for (int k = 0; k < iterations; k++) {
                for (int t = 0; t < 3; t++) {
            		...
            		"A.ZIP_CODE, A.CNTRY_CO, E.EM_EMAIL_LINE, I.bar_code, E.EM_EMAIL_TYPE, SI.ID from " +
            		term_stad + " A, (" + term_admn + " S LEFT JOIN " +
            		"sqldse.PRSNEMAD E ON S.PID9 = E.PID) LEFT JOIN " +
            		"affiliates_dw.rosetta_stone I ON S.PID9 = I.stu_pid LEFT JOIN affiliates_dw.affiliates_safe_attributes SA ON S.PID9 = SA.pid " +
                    "LEFT JOIN affiliates_dw.system SI ON SA.aid = SI.aid and SI.system_id = 41 " +
                    ...
                    String query = "select " + "S.PID9, " + "S.STUDENT_NAME, "
                            ...
                            + "(S.PID9 *= E.PID) and "
                            + "(S.PID9 *= I.pid) and " + "(S.REFRESH_DATE >= '"
                            + getYesterday() + "') "
                    ...
                    */
                    ...
                    try {
                        try {
                            ...
                            FileUtils.confirmDir(marcFilesDir);
                            FileUtils.stringToFile(query, marcFilesDir
                                    + "inc_query_grad_student_" + gradType + ".sql");
                        } catch (IOException ioe) {
                        }
